    ...
    body:BlocBuilder<ProfileBloc,ProfileState>(builder:(ctx,state){
      ...
      return ListView(padding:const EdgeInsets.all(D.md),children:[
        ...
        Center(child:Column(children:[
          Container(width:88,height:88,decoration:BoxDecoration(color:AppColors.primary,shape:BoxShape.circle,boxShadow:[BoxShadow(color:AppColors.primary.withOpacity(0.3),blurRadius:20,spreadRadius:4)]),
        ...
        const SizedBox(height:D.md),_Label('الدعم والمعلومات'),
        ...
        _Item(icon:Icons.info_outline_rounded,label:S.appVersion,trailing:const Text('2.0.0',style:TS.cap),onTap:null),
        ...
        AppCard(color:AppColors.errorBg,child:ListTile(
          ...
          onTap:()=>showDialog(context:ctx,builder:(_)=>AlertDialog(
            title:const Text('تأكيد الخروج'),content:Text(S.logoutConfirm),
            actions:[TextButton(onPressed:()=>Navigator.pop(ctx),child:const Text(S.cancel)),
              TextButton(onPressed:(){Navigator.pop(ctx);ctx.read<AuthBloc>().add(AuthLogoutEvent());},child:Text(S.logout,style:const TextStyle(color:AppColors.error))),
            ])))),
...
class _ChangePassState extends State<ChangePasswordScreen> {
  ...
  @override Widget build(BuildContext context)=>Scaffold(
    ...
    body:BlocConsumer<ProfileBloc,ProfileState>(
      ...
      builder:(ctx,s)=>Padding(padding:const EdgeInsets.all(D.xl),child:Form(key:_form,child:Column(children:[
